...
interface Cert { id?: number; name: string; issuer: string; year: string; url: string; display_order: number }
const empty: Cert = { name: "", issuer: "", year: "", url: "", display_order: 0 };
...
export default function CertificationsAdmin() {
  ...
  const Form = ({ item, onSave, onCancel }: { item: Cert; onSave: (i: Cert) => void; onCancel: () => void }) => {
    const [form, setForm] = useState(item);
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 space-y-4">
        <div className="grid sm:grid-cols-2 gap-4">
          {[{ k: "name", label: "Certification Name" }, { k: "issuer", label: "Issuer" }, { k: "year", label: "Year" }, { k: "url", label: "URL" }].map(({ k, label }) => (
            <div key={k}>
              <label className="block text-xs font-medium text-gray-600 mb-1">{label}</label>
              <input type="text" value={(form as never)[k] as string} onChange={(e) => setForm({ ...form, [k]: e.target.value })} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          ))}
        </div>
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold text-gray-900">Certifications</h1><p className="text-gray-500 text-sm">Manage your certifications</p></div>
        <button onClick={() => { setAdding(true); setEditing(null); }} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700"><Plus size={16} /> Add Certification</button>
      </div>
            ...
            {editing?.id === item.id ? (
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-indigo-600 text-sm">{item.issuer} · {item.year}</p>
                </div>
